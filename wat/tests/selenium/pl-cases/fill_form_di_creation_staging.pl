...
my $selectIndex = $sel->get_element_index("css=select[name=\"disk_image\"] option[value=\"ubuntu-13.04-i386-qvd.tar.gz\"]");
...
WAIT: {
    for (1..60) {
        if (eval { $sel->is_element_present("css=input[data-di-uploading=\"completed\"]") }) { pass; last WAIT }
        sleep(1);
    }
    fail("timeout");
}
WAIT: {
    for (1..60) {
        if (eval { $sel->is_element_present("css=tr[data-name=\"ubuntu-13.04-i386-qvd.tar.gz\"]") }) { pass; last WAIT }
        sleep(1);
    }
    fail("timeout");
}
