package QVD::Admin4::Parser;
use strict;
use warnings;
use Clone qw(clone);
use Moo;
use QVD::Admin4::Parser::Edge;
use QVD::Admin4::Parser::Agenda;
use QVD::Admin4::Parser::Chart;
use QVD::Admin4::Parser::Node;
use QVD::Admin4::Grammar::Substitution;
...
sub parse
{
    ...
    $LAST = 0;
    $self->{agenda} = QVD::Admin4::Parser::Agenda->new();
    $self->{chart} = QVD::Admin4::Parser::Chart->new();
    ...
    for my $edge (@{$self->chart->inactive_edges})
    {
		...
		# Creates an object QVD::Admin4::Grammar::Meaning
...
sub is_root
{
	...
	target_substitution => QVD::Admin4::Grammar::Substitution->new(), 
...
# a certain string
	...
	{
	    my $node = QVD::Admin4::Parser::Node->new(
		label => $label, 
		meaning => $meaning,
		substitution => QVD::Admin4::Grammar::Substitution->new()); 
	    my $edge = QVD::Admin4::Parser::Edge->new(
...
sub expand_edge
{
	...
	my $node = QVD::Admin4::Parser::Node->new(rule => $rule, substitution => $substitution); 

	# Created a new edge regarding the new constituent
	my $new_edge = QVD::Admin4::Parser::Edge->new(
...
sub combine_edge_aux 
{
    ...
    # Created a node that implements a new grammatical constituent. 
    # It will be the kind of constituent defined by the rule $rule
    my $node = QVD::Admin4::Parser::Node->new(rule => $active_edge->node->rule, substitution => $substitution); 
