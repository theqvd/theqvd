...
sub cfg {
    ...
    if ($USE_DB) {
        ...
        if ($tenant != -1) {
            ...
            TRACE "obtained value '$value' for key '$key' tenant_id '$tenant' obtained from database" if defined $value;
        }
        ...
        unless (defined $value) {
            $cfg || reload;
            $value = $cfg->{$key};
            TRACE "obtained value '$value' for key '$key' obtained from memory cache" if defined $value;
        }
    }
    ...
    unless (defined $value) {
        ...
        TRACE "obtained value '$value' for key '$key' obtained from the default values" if defined $value;
    }

    # Substitute any reference to another token in the configuration value
    if (defined $value) {
        $value =~ s/\$\{(.*?)\}/cfg($1, $tenant)/ge;
        TRACE sprintf "after dereferencing, value is '%s'", $value//'<undef>';
    }
    elsif ($mandatory) {
