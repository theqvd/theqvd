...
sub _config_cgroups {
    ...
    open (my $lxc_conf, "<", $lxc_conf_file) or DEBUG "Couldn't open $lxc_conf_file file: $!:, using default lxc config values.";
    ...
    INFO "Searching cgroup2 controllers...";
    ...
    for my $control (qw(memory cpu cpuset)) {
        if (/$control/i ~~ @controllers) {
            INFO "Controller $control is loaded into cgroup2 filesystem.";
        } else {
            die "Controller $control is not loaded into cgroup2 filesystem.";
        }
        if ($pattern) {
