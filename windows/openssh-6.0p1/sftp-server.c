...
sftp_server_main(int argc, char **argv, struct passwd *user_pw)
{
	...
	extern char *__progname;
        ...
        LPCTSTR named_pipe = NULL;    /* named pipe for receiving socket on Windows */
        LPCTSTR temp_file = NULL;     /* temp file for receiving socket on Windows */
        LPCTSTR log_file = NULL;      /* file for logging */

	__progname = ssh_get_progname(argv[0]);
	log_init(__progname, log_level, log_facility, log_stderr);

	while (!skipargs && (ch = getopt(argc, argv, "L:F:P:f:l:u:cehR")) != -1) {
		switch (ch) {
                case 'L':
                        /* Name of log file */
                        log_file = optarg;
	...
	log_init(__progname, log_level, log_facility, log_stderr);
        ...
        if (log_file) {
            set_log_handler(_log_to_file, log_file);
        }
