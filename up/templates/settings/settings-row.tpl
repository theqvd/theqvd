    ...
    <tr>
        <td class="cell-link">
            <a href="javascript:" class="js-button-settings-conf" data-id="<%= model.get('id') %>" data-i18n="[title]Edit">
                <%= model.get('name') %>
                <i class="<%= CLASS_ICON_EDIT %> fleft desktop"></i>
            </a>
        </td>
        <% if (model.get('settings') == null) { %>
            <td colspan="3" class="center desktop">
        ...
        <% } else { %>
            ...
            <td class="desktop"><a class="button-icon button-rounded js-active-workspace-btn <%= model.get('active') ? 'button button-active js-button-active' : 'button2 button-activatable js-button-activable' %> <%= CLASS_ICON_ACTIVE %>" data-id="<%= model.get('id') %>" data-i18n="[title]<%= model.get('active') ? 'Active configuration' : 'Set as current configuration' %>"></a></td>
            <td class="desktop">
                <% if(!model.get('fixed')) { %>
