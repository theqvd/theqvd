<div class="details-header">
    ...
    <% } %>
    <% if(Up.C.checkGroupACL('userEdit')) { %>
    ...
    <% 
    if (Up.C.checkACL('user.update.block')) {
    ...
    <% 
    if(Up.C.checkACL('vm.update.disconnect-user')) { 
        var enabled = false;
...
<table class="details details-list <% if (!enabledProperties) { %> col-width-100 <% } %>">
    ...
    if (Up.C.checkACL('user.see.description')) { 
    %>
        <tr>
    ...
    if (Up.C.checkACL('user.see.created-by')) {
    %>
        <tr>
    ...
    if (Up.C.checkACL('user.see.creation-date')) {
    %>
        <tr>
