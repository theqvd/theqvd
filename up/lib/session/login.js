Up.L = {
    ...
    logIn: function (sid, login) {
        ...
        if (Up.C.crossOrigin) {
            $.cookie('sid', sid, { expires: Up.C.loginExpirationDays, path: '/' });
        }
    ...
    isLogged: function () {
        ...
        if (Up.C.crossOrigin && (!$.cookie('sid') || $.cookie('sid') != Up.C.sid)) {
            cookieChecking = false;
        }
        
        if (Up.C.loggedIn && Up.C.sid != '' && Up.C.login != '' && cookieChecking) {
            return true;
        }
        else {
            Up.L.logOut();
    ...
    checkLogin: function (that) {
        ...
        if (that.retrievedData.status == STATUS_SUCCESS && that.retrievedData.statusText == 'error') {
            ...
            Up.L.logOut();
            Up.I.M.showMessage({message: "Wrong credentials", messageType: "error"});
            that.login = '';
            that.sid = '';
            return;
        }
        ...
        if (Up.CurrentView.qvdObj == 'login') {
            ...
            Up.Router.watRouter.performRoute('', Up.Views.HomeView);
        }
    ...
    afterLogin: function () {
        ...
        if (Up.Router.watRouter == undefined) {          
            ...
            Up.Router.watRouter = new Up.Router;
        }
    },
}
